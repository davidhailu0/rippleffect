...
const AuthSync = ({ children }: { children: React.ReactNode }) => {
  ...
  const [yellowBarConfig, setYellowBarConfig] = useState<NotificationBarProps>({
    message: "",
  });
  ...
  const checkAuth = useCallback(() => {
    const regex =
      /^\/work-with-me(?:\/(landing_pages\/\d+\/?)?)?(?:\?ref_code=[a-zA-Z0-9]+)?\/?$/;
  ...
  const checkBooking = useCallback(() => {
    ...
    if (daysDifference === 0) {
      if (hoursDifference >= 1) {
        setYellowBarConfig({
          message: `Your call is up in ${Math.floor(
            hoursDifference
          )} Hour(s) from now`,
        });
      ...
      setYellowBarConfig({
        message: "Your call is tomorrow. Please watch all videos in Step 3",
      });
    }
  }, [cookies]);
  ...
  return (
    <CookiesProvider defaultSetOptions={{ path: "/" }}>
      <VideoContext.Provider value={{ videos }}>
        <YellowNotificationBar
          message={yellowBarConfig.message}
          actionLabel={yellowBarConfig.actionLabel}
          onAction={yellowBarConfig.onAction}
        />
        {children}
