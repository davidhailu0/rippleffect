...
export default function SignUpForm({ ref_code }: { ref_code: string | undefined }) {
    ...
    if (state && (state.error || state.errors)) {
        toast.error(state.error || state.errors, { position: "top-center", })
    }
    ...
    return <>
        ...
        <form action={state && state.frontend_token ? confirmFormAction : formAction} className="flex flex-col w-11/12 md:w-1/3 gap-4 mx-auto">
            ...
            <TextField name={"token"} type={"text"} placeholder={"Confirmation Token"} hidden={!Boolean(state) || state.error || state.errors} />
            <button type={'submit'} className={`${(!Boolean(state) || state.error || state.errors) && 'hidden'} bg-gradient-to-r from-[#18455F] via-[#4B7A87] to-black h-12 rounded-[50px] text-white shadow-xl font-bold mt-6 col-span-2 w-[260px] mx-auto text-center`}>Confirm</button>
