...
import { fetchSurveys, answerSurvey } from '@/src/services/SurveyService';
import SurveyQuestion from './SurveyQuestion';
import Question from '@/src/types/Question';
import Answer from '@/src/types/Answers';

export default function DisplayQuestion() {
    const { data, isLoading } = useQuery<Question, Error>({ queryKey: ['surveys'], queryFn: fetchSurveys });
    ...
    useEffect(() => {
        const handleVisibilityChange = () => {
            ...
            if (videoElement) {
                document.visibilityState === 'hidden' ? videoElement.pause() : videoElement.play();
            }
        };
    ...
    const goToNext = useCallback(() => {
        ...
        if (index + 1 < data.questions.length) {
            ...
            if (nextResponse) {
                setSelectedValue({ question_id: data.questions[index + 1].id, response: nextResponse });
            }
        } else {
    ...
    if (isLoading) return <ClipLoader color="#fff" size={70} className="h-10 w-10" />;

    if (!data?.questions?.length) return <p className="text-white text-2xl font-bold">No Questions Found</p>;
    ...
    return (
        <div className="mt-7 md:mt-12 flex flex-col w-full md:w-2/5">
            <div className="h-auto w-full mx-auto flex flex-col gap-7 bg-white px-8 py-10">
                <Image src="/logo.webp" alt="logo" height={95} width={90} className="mx-auto" unoptimized />
                <MuxPlayer
                    ref={videoRef}
