...
export default function BookingComponent({
  ...
  const [selectedTimezone, setSelectedTimezone] =
    useState<TimezoneOption | null>(initialTimezone);
  ...
  const updateSelectedDate = useCallback(
    (availabilities: Availabilities | null) => {
      ...
      if (availabilities && availabilities[todayKey]) {
        const todayAvailable = availabilities[todayKey].some(
      ...
      const nextAvailableDate = Object.keys(availabilities || {})
        ...
        <Calendar
          ...
          tileContent={({ date }) =>
            isSameDay(date, new Date()) ? (
              <div
                className="h-2 w-2 absolute left-[44%] rounded-full bg-white"
                aria-hidden="true"
              ></div>
            ) : null
          }
          ...
          tileDisabled={({ activeStartDate, date }) =>
            ...
            !Object.keys(availabilities || {}).includes(
          ...
          {selectedDate && (
            <p className="text-black">
              <span className="font-bold text-lg">
                {DAYS[new Date(selectedDate.toString()).getDay()]}
              </span>{" "}
